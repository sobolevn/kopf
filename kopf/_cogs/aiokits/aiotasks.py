...
from typing import TYPE_CHECKING, Any, Awaitable, Callable, Collection, \
                   Coroutine, Generator, Optional, Set, Tuple, TypeVar, Union
...
class Scheduler:
    ...
    def __init__(
        ...
        self._pending_coros: asyncio.Queue[Tuple[Coroutine[Any, Any, Any], Optional[str]]] = asyncio.Queue()
    ...
    async def close(self) -> None:
        ...
        # Wait until all running and pending tasks are fully done (it can take some time).
        async with self._condition:
            await self._condition.wait_for(
                lambda: self._pending_coros.empty() and not self._running_tasks)
    ...
    async def spawn(
        ...
        # Even if being closed, accept new coros --- they are already created and need a cleanup.
        # But not when the scheduler is really shut down and nothing is going to happen anymore.
        if self._spawning_task.done() or self._cleaning_task.done():
            raise RuntimeError("Cannot add new coroutines to a closed and inactive scheduler.")
        async with self._condition:
            await self._pending_coros.put((coro, name))
    ...
    async def _task_spawner(self) -> None:
        while True:
            async with self._condition:
                ...
                while not self._pending_coros.empty() and (self._limit is None or len(self._running_tasks) < self._limit):
